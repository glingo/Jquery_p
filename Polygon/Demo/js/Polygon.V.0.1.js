(function ($) {
        ...
        floor: function (length) {
            ...
            for (i = 1; i < vision * 2; i++) {
                ...
                for (j = 1; j < vision * 2; j++) {
                    ...
                    path.push(origin.add(ref));
                }
            }

            $.each(path, function (index, vector) {
                var o = vector.to2d(camera, canvas),
                    a = vector.add(new Vector(length, 0, 0)).to2d(camera, canvas),
                    b = vector.add(new Vector(0, 0, length)).to2d(camera, canvas);
    ...
    Scene.prototype.generateAxes = function () {
        ...
        $.each(this.axes, function (name, axe) {
            var a = new Vector(150, 150, 0).to2d(camera, canvas),
                b = new Vector(150, 150, 0).add(axe).to2d(camera, canvas);
